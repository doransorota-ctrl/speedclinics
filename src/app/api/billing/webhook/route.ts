...
async function handleRegularSignup(
  ...
  if (business) {
    ...
    await assignOrBuyNumber(business.id, metadata.businessName || "Clŷniq").catch((err) => {
  ...
  await sendEmail({
    to: email,
    subject: `Welkom bij Clŷniq, ${metadata.firstName}!`,
...
async function handleGoogleSignupComplete(
  ...
  if (businessId) {
    ...
    if (googleBiz && !googleBiz.twilio_number) {
      const { assignOrBuyNumber } = await import("@/lib/twilio/pool");
      await assignOrBuyNumber(businessId, googleBiz.name || "Clŷniq").catch((err) => {
  ...
  await sendEmail({
    to: email,
    subject: `Welkom bij Clŷniq${firstName !== "daar" ? `, ${firstName}` : ""}!`,
    html: welcomeEmailHtml({
      firstName,
      businessName,
      portalUrl: `${APP_URL}/login`,
