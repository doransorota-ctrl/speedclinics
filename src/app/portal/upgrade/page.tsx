    ...
    return (
      <div>
        ...
        <div className="mt-6 bg-white rounded-xl border border-surface-200 p-8 text-center">
          ...
          <p className="text-sm text-surface-500 mt-1">Je hebt toegang tot alle functies van Speed Clinics.</p>
        </div>
      </div>
    );
  }
  ...
  return (
    <div>
      ...
      <p className="text-surface-500 mt-1">
        Je hebt nu het {isSpeedLeads ? "Speed Clinics" : "Website"} pakket. Kies een upgrade.
      </p>

      {error && <p className="text-sm text-red-600 mt-4">{error}</p>}

      <div className={`mt-6 grid grid-cols-1 ${isSpeedLeads || isWebsite ? "md:grid-cols-2" : ""} gap-4`}>
        {/* Speed Clinics card — shown to website users */}
        {isWebsite && (
          <div className="bg-white rounded-2xl border-2 border-brand-500 shadow-lg p-6 flex flex-col">
            ...
            <h3 className="text-lg font-bold text-surface-900">Speed Clinics</h3>
            <div className="mt-2">
            ...
            >
              {upgrading ? "Bezig..." : "Voeg Speed Clinics toe"}
            </button>
          </div>
        )}
        ...
        <div className="relative bg-white rounded-2xl border border-surface-200 p-6 flex flex-col">
          ...
          <ul className="mt-4 space-y-2 flex-1">
            {["Alles van Speed Clinics", "Alles van Website Pakket", "Website + leadopvolging in \u00e9\u00e9n", "E\u00e9n factuur, geen gedoe"].map((f) => (
              <li key={f} className="flex items-start gap-2 text-sm text-surface-600">
                {FEATURE_CHECK}
                {f}
              </li>
